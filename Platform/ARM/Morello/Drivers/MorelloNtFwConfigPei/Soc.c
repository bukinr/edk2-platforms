  ...
  )
{
  ...
  PlatInfo->RemoteDdrSize = fdt64_to_cpu (ReadUnaligned64 (DdrProperty));
  ...
  PlatInfo->RemoteChipCount = fdt32_to_cpu (*Property);
  ...
  PlatInfo->Mode = fdt32_to_cpu (*Property);

  Property = fdt_getprop (ParamPpi->NtFwConfig, Offset, "scc-config", NULL);
  if (Property == NULL) {
    DEBUG ((
      DEBUG_ERROR,
  ...
  PlatInfo->SccConfig = fdt32_to_cpu (*Property);

  gPpi.Flags = EFI_PEI_PPI_DESCRIPTOR_PPI
               | EFI_PEI_PPI_DESCRIPTOR_TERMINATE_LIST;
  gPpi.Guid = &gArmMorelloSocPlatformInfoDescriptorPpiGuid;
  gPpi.Ppi  = PlatInfo;
